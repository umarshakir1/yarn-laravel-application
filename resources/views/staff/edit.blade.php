    ...
    <x-slot name="header">
        <div class="flex items-center gap-3">
            ...
            <div>
                ...
                <p class="text-sm text-gray-500 mt-0.5">Update account details and section access for <span class="font-semibold text-gray-700">{{ $staff->name }}</span></p>
            </div>
        </div>
    </x-slot>

    <div class="py-8 px-6 lg:px-8 w-full">
        <form method="POST" action="{{ route('staff.update', $staff) }}" class="max-w-2xl space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
                ...
                <div class="space-y-4">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        ...
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1.5" for="password_confirmation">Confirm New Password</label>
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
                ...
                <p class="text-xs text-gray-500 mb-4">Check the sections this staff member is allowed to access. Changes take effect immediately on save.</p>

                <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    @foreach($sections as $section)
                        @php
                            $isChecked = in_array($section['permission'], old('permissions', $userPermissions));
                        @endphp
                        <label class="flex items-center gap-3 p-3 rounded-lg border cursor-pointer transition-all
                                      {{ $isChecked ? 'border-indigo-400 bg-indigo-50' : 'border-gray-200 hover:border-indigo-300 hover:bg-indigo-50/40' }} group">
                            <input type="checkbox" name="permissions[]" value="{{ $section['permission'] }}"
                                   ...
                                   {{ $isChecked ? 'checked' : '' }}
                                   onchange="this.closest('label').classList.toggle('border-indigo-400', this.checked); this.closest('label').classList.toggle('bg-indigo-50', this.checked); this.closest('label').classList.toggle('border-gray-200', !this.checked);">
                            <div class="flex items-center gap-2 min-w-0">
                                <div class="w-7 h-7 rounded-md {{ $isChecked ? 'bg-indigo-100' : 'bg-gray-100' }} flex items-center justify-center shrink-0">
                                    <svg class="w-4 h-4 {{ $isChecked ? 'text-indigo-600' : 'text-gray-500' }}" fill="none" viewBox="0 0 24 24" stroke="currentColor">
    ...
    <script>
        function setAll(checked) {
            document.querySelectorAll('input[name="permissions[]"]').forEach(cb => {
                ...
                label.classList.toggle('border-indigo-400', checked);
