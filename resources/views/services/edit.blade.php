<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-2xl font-bold text-gray-900">Edit Service: {{ $service->name }}</h1>
                <p class="text-sm text-gray-500 mt-0.5">Update the details for this add-on service</p>
            </div>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-8">
            <form action="{{ route('services.update', $service) }}" method="POST" class="space-y-6">
                ...
                <div>
                    ...
                    <input id="name" name="name" type="text" autofocus required value="{{ old('name', $service->name) }}"
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div>
                        ...
                        <input id="price" name="price" type="number" step="0.01" min="0" required value="{{ old('price', $service->price) }}"
                    ...
                    <div>
                        ...
                        <input id="cost_price" name="cost_price" type="number" step="0.01" min="0" required value="{{ old('cost_price', $service->cost_price) }}"
                               class="w-full rounded-lg border border-gray-300 px-4 py-2.5 text-sm font-mono focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200 outline-none transition-colors">
                    ...
                    <div>
                        ...
                        <select id="unit" name="unit" required
                                class="w-full rounded-lg border border-gray-300 px-4 py-2.5 text-sm focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200 outline-none transition-colors">
                            <option value="per_kg" {{ old('unit', $service->unit) == 'per_kg' ? 'selected' : '' }}>per KG</option>
                            <option value="per_bundle" {{ old('unit', $service->unit) == 'per_bundle' ? 'selected' : '' }}>per Bundle</option>
                            <option value="per_bag" {{ old('unit', $service->unit) == 'per_bag' ? 'selected' : '' }}>per Bag</option>
                        </select>
                ...
                <div>
                    ...
                    <textarea id="description" name="description" rows="3"
                              class="w-full rounded-lg border border-gray-300 px-4 py-2.5 text-sm focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200 outline-none transition-colors">{{ old('description', $service->description) }}</textarea>
