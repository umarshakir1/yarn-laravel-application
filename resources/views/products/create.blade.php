            ...
            <form action="{{ route('products.store') }}" method="POST" class="space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input id="quality" name="quality" type="text" value="{{ old('quality') }}" required
                    ...
                    <div>
                        ...
                        <select id="unit_type" name="unit_type" required
                            ...
                            <option value="per_bag" {{ old('unit_type', 'per_bag') == 'per_bag' ? 'selected' : '' }}>Per Bag</option>
                            <option value="per_kg" {{ old('unit_type') == 'per_kg' ? 'selected' : '' }}>Per KG</option>
                        </select>
                        @error('unit_type')
                            <p class="mt-1.5 text-xs text-red-600">{{ $message }}</p>
                        @enderror
                ...
                <div>
                    ...
                    <textarea id="description" name="description" rows="3"
                              class="w-full rounded-lg border border-gray-300 px-4 py-2.5 text-sm focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200 transition-colors outline-none"
                ...
                <div class="flex items-center gap-3 pt-2">
                    ...
                    <a href="{{ route('products.index') }}" class="px-6 py-2.5 bg-gray-100 text-gray-700 text-sm font-semibold rounded-lg hover:bg-gray-200 transition-colors">
                        Cancel
                    </a>
                </div>
            </form>
